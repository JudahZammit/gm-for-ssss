from models.caerus_skip_func_model import SkipFuncCaerus
...
def display_val(display_list,epoch):
  ...
  for i in range(len(display_list)):
    ...
    plt.axis('off')
  plt.savefig("./model_output/SkipFuncCaerus/epoch_"+str(epoch)+".png")
...
def show_val_predictions(epoch):
    ...
    display_val([point[0][0],create_mask(point[1][0]) ,
        create_mask(out['y_s_reconstructed'][0]), 
        out['x_s_reconstructed'][0],
        create_mask(out['y_u_reconstructed'][0])],
        epoch)
...
def show_train_predictions():
    ...
    out = model.predict_on_batch(point)
    display_train([point[0][0],
        create_mask(point[1][0]) ,
        create_mask(out['y_s_reconstructed'][0]), 
        out['x_s_reconstructed'][0],
        point[2][0],
        out['x_u_reconstructed'][0],
        create_mask(out['y_u_reconstructed'][0])])
...
checkpoint = ModelCheckpoint(
    './model_output/SkipFuncCaerus/SkipFuncCaerus.tf', 
    monitor='val_IOU', 
    verbose=0, 
...
for i in range(0,201):
    ...
    model = SkipFuncCaerus(Batch_Norm = BN,Kull = KL) 
    model.compile(opt)  
     
    if KL>0:
        nex = next(train_gen)
        model.train_on_batch(nex[0],nex[1])
        model.load_weights('./model_output/SkipFuncCaerus/SkipFuncCaerus.tf')
    if(KL == 1):
        epochs = 100000
    elif(KL == 0):
        epochs = 10

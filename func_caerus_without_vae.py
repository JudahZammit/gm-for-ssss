from caerus_model import FuncCaerusWithoutVAE
...
for i in range(100):
    ...
    model = FuncCaerusWithoutVAE(Batch_Norm = BN,Kull = KL) 
    model.compile(opt)  
     
    if i > 0:
        nex = next(train_gen)
        model.train_on_batch(nex[0],nex[1])
        model.load_weights('FuncCaerusWithoutVAE.h5')
    if(KL == 1):
        epochs = 100
    else:
        epochs = 1
